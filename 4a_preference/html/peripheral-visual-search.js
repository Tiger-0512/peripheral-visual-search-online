...
function PracticeTrials1LoopBegin(PracticeTrials1LoopScheduler) {
  ...
  for (const thisPracticeTrials1 of PracticeTrials1) {
...
function ActualTrials1LoopBegin(ActualTrials1LoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisActualTrials1 of ActualTrials1) {
...
function PracticeTrials2LoopBegin(PracticeTrials2LoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisPracticeTrials2 of PracticeTrials2) {
...
function rankImages_pLoopBegin(rankImages_pLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisRankImages_p of rankImages_p) {
...
function ActualTrials2LoopBegin(ActualTrials2LoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisActualTrials2 of ActualTrials2) {
...
function rankImages_aLoopBegin(rankImages_aLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisRankImages_a of rankImages_a) {
...
function showStim1RoutineBegin(snapshot) {
  return function () {
    ...
    if (currentLoop.name == "PracticeTrials1") {
        ...
        if (magnified === 1) {
            image_path = `imagenet/1/magnified/${num_to_class[presented_class]}/${num_to_class[presented_class]}_${image_n}.png`;
        } else {
            image_path = `imagenet/1/standard/${num_to_class[presented_class]}/${num_to_class[presented_class]}_1${image_n}.png`;
        }
    }
    ...
    for (const thisComponent of showStim1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function askQuestion1RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of askQuestion1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function actualIntro1RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of actualIntro1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function expIntro2RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of expIntro2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function initializeTrialRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of initializeTrialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function showStim2RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of showStim2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function showStim2RoutineEachFrame(snapshot) {
  return function () {
    ...
    if ((initial_flag && (showStim2Clock.getTime() > 6))) {
        document.body.style.cursor='auto';
...
function updateSquaresRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of updateSquaresComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function actualIntro2RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of actualIntro2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
