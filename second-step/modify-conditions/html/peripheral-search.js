...
function showStimRoutineBegin(snapshot) {
  return function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    if ((currentLoop.name == "ActualTrials") && ((count % 48) === 0)) {
        if (count == 288) {
            idx = range(101, 136);
            idx = idx.map(x => x.toString().slice(1));
            idx_list = {};
    ...
    for (var i = 0, _pj_a = image_list.length; (i < _pj_a); i += 1) {
        if (currentLoop.name == "PracticeTrials") {
            ...
            image_path = "imagenet/" + _non_target_classes[i] + "/image" + idx_list[_non_target_classes[i]][count % 48] + ".png";
        }
    ...
    if (currentLoop.name == "PracticeTrials") {
        ...
        image_path = "imagenet/" + target_class + "/image" + idx_list[target_class][count % 48] + ".png";
    }
