...
test = visual.TextStim(win=win, name='test',
    text='Any text',
    font='Open Sans',
    pos=(0, 0), height=deg2norm * 0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=-3.0);

# Initialize components for Routine "show_stim_p"
show_stim_pClock = core.Clock()
...
# Initialize components for Routine "show_stim_a"
show_stim_aClock = core.Clock()
show_stim_key_resp_2 = keyboard.Keyboard()
...
for thisPracticeTrial in PracticeTrials:
    ...
    # ------Prepare to start Routine "show_stim_p"-------
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # keep track of which components have finished
    show_stim_pComponents = [show_stim_key_resp]
    for thisComponent in show_stim_pComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    show_stim_pClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "show_stim_p"-------
    while continueRoutine:
        # get current time
        t = show_stim_pClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=show_stim_pClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        fixation_point.draw()
        
        if show_stim_pClock.getTime() > 1:
            for i in range(len(image_list)):
                image_list[i].draw()
    ...
    # -------Ending Routine "show_stim_p"-------
    for thisComponent in show_stim_pComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('reactionTime', show_stim_pClock.getTime() - 1)
    # the Routine "show_stim_p" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    continueRoutine = True
    # update component parameters for each repeat
    if key_to_pos[key_ans.keys][0] == thisPracticeTrial['pos'] and key_to_pos[key_ans.keys][1] == thisPracticeTrial['ori']:
...
for thisActualTrial in ActualTrials:
    ...
    # ------Prepare to start Routine "show_stim_a"-------
    continueRoutine = True
    # update component parameters for each repeat
    _non_target_classes = random.sample(non_target_classes, len(non_target_classes))
    for i in range(len(image_list)):
    ...
    show_stim_key_resp_2.keys = []
    show_stim_key_resp_2.rt = []
    _show_stim_key_resp_2_allKeys = []
    # keep track of which components have finished
    show_stim_aComponents = [show_stim_key_resp_2]
    for thisComponent in show_stim_aComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    t = 0
    _timeToFirstFrame = win.getFutureFlipTime(clock="now")
    show_stim_aClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
    frameN = -1
    
    # -------Run Routine "show_stim_a"-------
    while continueRoutine:
        # get current time
        t = show_stim_aClock.getTime()
        tThisFlip = win.getFutureFlipTime(clock=show_stim_aClock)
        tThisFlipGlobal = win.getFutureFlipTime(clock=None)
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        fixation_point.draw()
        
        if show_stim_aClock.getTime() > 1:
            for i in range(len(image_list)):
                image_list[i].draw()
        
        
        # *show_stim_key_resp_2* updates
        waitOnFlip = False
        if show_stim_key_resp_2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
            # keep track of start time/frame for later
            show_stim_key_resp_2.frameNStart = frameN  # exact frame index
            show_stim_key_resp_2.tStart = t  # local t and not account for scr refresh
            show_stim_key_resp_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(show_stim_key_resp_2, 'tStartRefresh')  # time at next scr refresh
            show_stim_key_resp_2.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(show_stim_key_resp_2.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(show_stim_key_resp_2.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if show_stim_key_resp_2.status == STARTED and not waitOnFlip:
            theseKeys = show_stim_key_resp_2.getKeys(keyList=None, waitRelease=False)
            _show_stim_key_resp_2_allKeys.extend(theseKeys)
            if len(_show_stim_key_resp_2_allKeys):
                show_stim_key_resp_2.keys = _show_stim_key_resp_2_allKeys[-1].name  # just the last key pressed
                show_stim_key_resp_2.rt = _show_stim_key_resp_2_allKeys[-1].rt
                # a response ends the routine
                continueRoutine = False
        if show_stim_aClock.getTime() > 7:
            continueRoutine=False
    ...
    # -------Ending Routine "show_stim_a"-------
    for thisComponent in show_stim_aComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('reactionTime', show_stim_pClock.getTime() - 1)
    # the Routine "show_stim_a" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
